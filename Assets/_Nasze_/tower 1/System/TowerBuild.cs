...
public class TowerBuild : MonoBehaviour, IUseMode
{
    ...
    private bool TryResolveMoneyModel()
    {
        ...
        if (moneyBridge == null)
        {
            ...
            if (moneyBridge == null)
            {
                Debug.LogError("TowerBuild: MoneyBridge not found in scene.");
                return false;
            }
        }
        ...
        if (money == null)
        {
            Debug.LogWarning("TowerBuild: MoneyModel is not initialized yet (waiting for MoneyInstaller).");
            return false;
        }
